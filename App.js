...
import { LeftMenuProvider, LeftMenuContext } from './Contexts/LeftMenuContext';
import { RightMenuContext, RightMenuProvider } from './Contexts/RightMenuContext'

const AppRoot = () => {
  const { isOpen, setIsOpen } = useContext(LeftMenuContext);
  const { isRightOpen, setIsRightOpen } = useContext(RightMenuContext);
  return (
    <SideMenu
      menu={<Left />}
      menuPosition='left'
